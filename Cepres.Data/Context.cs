...
namespace Cepres.Data
{
    public partial class Context : DbContext, IDbContext
    {
        ...
        public virtual List<TQuery> ExecuteStoredProc<TQuery>(string spName, List<SqlParameter> sqlParameters)
        {
            using DbCommand command = Database.GetDbConnection().CreateCommand();
            command.CommandText = spName;
            command.CommandType = CommandType.StoredProcedure;
            sqlParameters?.ForEach(f => command.Parameters.Add(f));
            if (command.Connection.State != ConnectionState.Open)
            {
                command.Connection.Open();
            }
            List<TQuery> result = new List<TQuery>();
            using (DbDataReader reader = command.ExecuteReader())
            {
                DataTable table = new DataTable();
                table.Load(reader);
                result = table.ToList<TQuery>();
            }
            return result;
        }
        public virtual int ExecuteStoredProc(string spName, List<SqlParameter> sqlParameters)
        {
            using DbCommand command = Database.GetDbConnection().CreateCommand();
            command.CommandText = spName;
            command.CommandType = CommandType.StoredProcedure;
